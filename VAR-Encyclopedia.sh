...
function getCreationName() {
	...
	local overrideDecision
		...
		if [[ $overrideDecision =~ (^([yY])$){1} ]] || [[ $overrideDecision =~ ^([yY][eE][sS])$ ]]; then
...
function deleteCreation() {
	...
	#If there are creations
		...
		if [[ $? -eq 0 ]]; then
                        ...
                        if [[ $deletionConfirmation =~ (^([yY])$){1} ]] || [[ $deletionConfirmation =~ ^([yY][eE][sS])$ ]]; then

				echo "${creationsArray[$creationIndex-1]}.avi has been deleted."
				echo
				...
				echo "You have let ${creationsArray[$creationIndex-1]}.avi exist a while longer."
				echo
			fi
		fi
        fi
